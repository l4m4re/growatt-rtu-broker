...
import abc, json, asyncio, time, pathlib, io
...
class Backend(abc.ABC):
    ...
    @abc.abstractmethod
    async def read_holding(self, unit: int, address: int, count: int) -> List[Number]: ...
    @abc.abstractmethod
    async def write_single(self, unit: int, address: int, value: Number) -> None: ...
    @abc.abstractmethod
    async def write_multiple(self, unit: int, address: int, values: Iterable[Number]) -> None: ...
...
class DatasetBackend(Backend):
    ...
    """
    def __init__(self, dataset_path: str | pathlib.Path, *, mutate: bool = False) -> None:
        p = pathlib.Path(dataset_path)
        raw = json.loads(p.read_text(encoding="utf-8"))
        self.holding: Dict[int, int] = {int(k): int(v) for k, v in raw.get("holding", {}).items()}
        self.input: Dict[int, int] = {int(k): int(v) for k, v in raw.get("input", {}).items()}
        self._mutate = mutate
    ...
    async def read_input(self, unit: int, address: int, count: int) -> List[int]:
        return [self._maybe_mutate(self.input, a, self.input.get(a, 0)) for a in range(address, address + count)]

    async def read_holding(self, unit: int, address: int, count: int) -> List[int]:
        return [self._maybe_mutate(self.holding, a, self.holding.get(a, 0)) for a in range(address, address + count)]

    async def write_single(self, unit: int, address: int, value: int) -> None:  # allow ephemeral overriding
        self.holding[address] = value & 0xFFFF

    async def write_multiple(self, unit: int, address: int, values: Iterable[int]) -> None:
...
class CaptureBackend(Backend):
    ...
    async def read_input(self, unit: int, address: int, count: int) -> List[int]:
        ...
        await self._log({"ts": time.time(), "op": "read_input", "unit": unit, "addr": address, "count": count, "regs": regs})
        return regs

    async def read_holding(self, unit: int, address: int, count: int) -> List[int]:
        regs = await self._inner.read_holding(unit, address, count)
        await self._log({"ts": time.time(), "op": "read_holding", "unit": unit, "addr": address, "count": count, "regs": regs})
        return regs

    async def write_single(self, unit: int, address: int, value: int) -> None:
        await self._inner.write_single(unit, address, value)
        await self._log({"ts": time.time(), "op": "write_single", "unit": unit, "addr": address, "value": value & 0xFFFF})

    async def write_multiple(self, unit: int, address: int, values: Iterable[int]) -> None:
        vals = [int(v) & 0xFFFF for v in values]
        await self._inner.write_multiple(unit, address, vals)
        await self._log({"ts": time.time(), "op": "write_multiple", "unit": unit, "addr": address, "values": vals})
